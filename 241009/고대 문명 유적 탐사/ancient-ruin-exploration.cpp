...
int BFS(vector<vector<int>>& m, int x, int y, int piece_value, vector<pair<int, int>>& visited_position, vector<vector<bool>>& visited) {
    visited[x][y] = true;
    m[x][y] = 0; 
    int count = 1;
    ...
    while (!q.empty()) 
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (nx >= 0 && ny >= 0 && nx < 5 && ny < 5 && !visited[nx][ny] && m[nx][ny] == piece_value) 
            {
                visited[nx][ny] = true;
                q.push({nx, ny});
...
int FindPiece(vector<vector<int>>& m) {
    ...
    vector<vector<bool>> visited(5, vector<bool>(5, false));  // visited
    ...
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 5; ++j) {
            ...
            {  
                ...
                int value = m[i][j];
                int count = BFS(m, i, j, value, visited_position, visited);

                if (count >= 3) {
                    find_piece_count += count;
                } else {
                    for (auto [x, y] : visited_position) {
                        m[x][y] = value;
                    }
                }
            }
